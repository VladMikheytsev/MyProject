        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center p-4 z-50">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 animate-fade-in-up">
                <div ref={qrCodePrintRef} className="text-center p-4 flex flex-col items-center">
                    ...
                    {item.uniqueId && <p className="text-sm text-gray-500 mb-2">{item.uniqueId.slice(0, 4)} {item.uniqueId.slice(4)}</p>}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-start overflow-y-auto p-4 z-50">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 animate-fade-in-up my-auto">
                ...
                <div className="space-y-4">
                    ...
                    {userData.uniqueId && (
                         <p className="text-sm text-gray-500 text-center">Ваш уникальный ID: <span className="font-mono">{userData.uniqueId.slice(0, 4)} {userData.uniqueId.slice(4)}</span></p>
                    )}
...
const ItemEditor = ({ warehouses, itemTypes, onSave, onCancel, onManageTypes, items, userRole }) => {
    ...
    const [newItem, setNewItem] = useState({ name: '', type: itemTypes[0]?.name || '', size: 'Паллета', quantity: 1, warehouseId: warehouses[0]?.id || null, placeId: null, uniqueId: generateUniqueId() });
                                    ...
                                    <div className="space-y-3">
                                        {warehouseData.users.map(user => (
                                            <div key={user.id} className="bg-gray-50 p-3 rounded-lg">
                                                ...
                                                {user.uniqueId && <p className="text-xs text-gray-500">{user.uniqueId.slice(0, 4)} {user.uniqueId.slice(4)}</p>}
                             ...
                             <div>
                                ...
                                <div className="max-h-40 overflow-y-auto space-y-2 p-2 bg-gray-50 rounded-lg">
                                    {drivers.map(d => (
                                        ...
                                        >
                                            {d.firstName} {d.lastName} {d.uniqueId && <span className="text-xs opacity-75">{d.uniqueId.slice(0, 4)} {d.uniqueId.slice(4)}</span>}
                                        </button>
                                    ))}
                                </div>
                            </div>
...
const RegisterView = ({ onRegister, onSwitchToLogin, warehouses }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await onRegister({...formData, uniqueId});
        } catch (err) {
...
const ScenarioPrintDocument = React.forwardRef(({ scenario, warehouses, items, users, signatures }, ref) => {
    ...
    // Helper to get user's unique ID
    const getUserUniqueId = (userId) => {
        if (!userId) return '';
        const user = users.find(u => u.id === userId);
        return user?.uniqueId ? `${user.uniqueId.slice(0, 4)} ${user.uniqueId.slice(4)}` : '';
    };
    ...
    const signatureLineStyle = {
        display: 'flex',
        flexDirection: 'column', // Changed to column to stack name and uniqueId
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid black', fontSize: '12px' }}>
                    <thead>
                        <tr>
                            ...
                            <th style={{ border: '1px solid black', padding: '8px' }}>Unique ID</th> {/* Added Unique ID column */}
                        ...
                        {Object.entries(scenario.items).map(([itemId, quantity]) => {
                            ...
                            return item ? (
                                <tr key={itemId}>
                                    ...
                                    <td style={{ border: '1px solid black', padding: '8px' }}>{item.uniqueId ? `${item.uniqueId.slice(0, 4)} ${item.uniqueId.slice(4)}` : ''}</td> {/* Display unique ID */}
                ...
                <div style={{ marginBottom: '1em' }}>
                    <div style={signatureLineStyle}>
                        <div style={{display: 'flex', alignItems: 'flex-end', gap: '30px', width: '100%'}}>
                            ...
                            <span>{getUserUniqueId(scenario.creatorId)}</span>
                        </div>
                        ...
                        <span style={{ marginLeft: 'auto', marginTop: 'auto' }}>{currentDate}</span>
                    </div>
                </div>
                <div style={{ marginBottom: '1em' }}>
                    <div style={signatureLineStyle}>
                        <div style={{display: 'flex', alignItems: 'flex-end', gap: '30px', width: '100%'}}>
                            ...
                            <span>{getUserUniqueId(scenario.driverId)}</span>
                        </div>
                ...
                <div>
                    <div style={signatureLineStyle}>
                        <div style={{display: 'flex', alignItems: 'flex-end', gap: '30px', width: '100%'}}>
                            ...
                            <span>{getUserUniqueId(scenario.completerId)}</span>
                        </div>
...
const LogModal = ({ log, users, onClose }) => {
    const getUserName = (userId) => {
        const user = users.find(u => u.id === userId);
        return user ? `${user.firstName} ${user.lastName} ${user.uniqueId ? `(${user.uniqueId.slice(0, 4)} ${user.uniqueId.slice(4)})` : ''}` : 'Неизвестный пользователь';
    };
...
export default function App() {
  ...
  const handleRegister = async (formData) => {
      // Generate an 8-digit unique ID
      const uniqueId = Math.floor(10000000 + Math.random() * 90000000).toString();
      const newUser = await api.registerUser({...formData, uniqueId});
      const now = new Date().getTime();
                                                 ...
                                                 return (
                                                     ...
                                                     >
                                                         <div className="flex items-start gap-3 flex-grow" onClick={() => { if (userRole === 'Администратор' && !isLocked) { setEditingItem(item); } }}>
                                                             ...
                                                             <div>
                                                                 <p className="font-bold text-gray-800">{item.name}</p>
                                                                 {item.uniqueId && <p className="text-xs text-gray-500">{item.uniqueId.slice(0, 4)} {item.uniqueId.slice(4)}</p>}
                                                                 <p className="text-sm text-gray-600">Тип: {item.type} | Размер: {item.size} | Кол-во: {item.quantity}</p>
                                                                 {isUnplaced ? (
...
}dium text-gray-700 mb-1">Количество:</label>
                            <input
                                ref={quantityInputRef}
                                type="number"
                                value={moveQuantity}
